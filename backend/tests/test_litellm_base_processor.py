...
@pytest.mark.django_db
def test_tools_enabled_empty_list(mock_settings):  # pylint: disable=unused-argument
    """
    Test that empty tools_enabled list does not add tools to extra_params.
    """
    config = {
        "LitellmProcessor": {
            "tools_enabled": [],
...
@pytest.mark.django_db
def test_tools_enabled_not_provided(mock_settings):  # pylint: disable=unused-argument
    """
    Test that when tools_enabled is not provided, tools are not added.
    """
    config = {
        "LitellmProcessor": {}
    }
...
@pytest.mark.django_db
def test_tools_enabled_single_function(mock_settings):  # pylint: disable=unused-argument
    """
    Test that a single function in tools_enabled is properly configured.
    """
    config = {
        "LitellmProcessor": {
            "tools_enabled": ["roll_dice"],
        }
    }
    processor = LitellmProcessor(config=config, user_session=None)
    ...
    assert processor.extra_params["tools"][0]["function"] == FUNCTIONS_SCHEMA["roll_dice"]
...
@pytest.mark.django_db
def test_tools_enabled_multiple_functions(mock_settings):  # pylint: disable=unused-argument
    """
    Test that multiple functions in tools_enabled are properly configured.
    """
    config = {
        "LitellmProcessor": {
            "tools_enabled": ["roll_dice", "get_location_content"],
        }
    }
    processor = LitellmProcessor(config=config, user_session=None)
    ...
    for tool in processor.extra_params["tools"]:
        ...
        assert tool["function"] in FUNCTIONS_SCHEMA.values()
...
@pytest.mark.django_db
def test_tools_enabled_filters_only_enabled_functions(mock_settings):  # pylint: disable=unused-argument
    """
    Test that only functions listed in tools_enabled are included.
    """
    config = {
        "LitellmProcessor": {
            "tools_enabled": ["roll_dice"],
    ...
    for tool in processor.extra_params["tools"]:
        assert tool["function"] != FUNCTIONS_SCHEMA["get_location_content"]
...
@pytest.mark.django_db
def test_tools_enabled_nonexistent_function(mock_settings):  # pylint: disable=unused-argument
    """
    Test that nonexistent functions in tools_enabled are silently ignored.
    """
    config = {
        "LitellmProcessor": {
            "tools_enabled": ["roll_dice", "nonexistent_function"],
        }
    }
    processor = LitellmProcessor(config=config, user_session=None)
...
@pytest.mark.django_db
def test_tools_enabled_all_available_functions(mock_settings):  # pylint: disable=unused-argument
    """
    Test that all available functions can be enabled.
    """
    all_function_names = list(FUNCTIONS_SCHEMA.keys())
    config = {
        "LitellmProcessor": {
            "tools_enabled": all_function_names,
        }
    }
    processor = LitellmProcessor(config=config, user_session=None)

    assert "tools" in processor.extra_params
    assert len(processor.extra_params["tools"]) == len(FUNCTIONS_SCHEMA)

    # Verify all functions are included
    included_schemas = [tool["function"] for tool in processor.extra_params["tools"]]
    for schema in FUNCTIONS_SCHEMA.values():
