...
└── tests/integration/
    ...
    └── test_profile_coverage.py       # profile coverage (unlettered, see below)
```
...
| R | `test_anthropic_cache_short_prompt_no_crash` | Anthropic only | No crash; valid response returned when prompt is below claude-haiku-4-5's 4096-token cache minimum | Anthropic silently rejects cache for prompts below the model's minimum |
...
#### Profile coverage (`test_profile_coverage.py`)

Unlettered tests exercising profiles not covered by the lettered matrix above, sending real HTTP requests through the full orchestrator → processor → LLM chain:

| Test name | Providers | What is validated |
|-----------|-----------|--------------------|
| `test_custom_prompt_profile_returns_rephrased_content` | OpenAI, Anthropic | `base/custom_prompt.json` (`call_with_custom_prompt` / `DirectLLMResponse`) returns a non-empty completed response using the profile's inline prompt |
| `test_library_creator_profile_returns_quiz_problems` | OpenAI, Anthropic | `base/library_questions_creator.json` via `EducatorAssistantOrchestrator` returns a `question_slots` list with at least one well-formed problem |
| `test_box_hello_profile_returns_greeting` | OpenAI only | `examples/openai/box_hello.json` (`greet_from_llm` / `DirectLLMResponse`) returns a non-empty response |
| `test_chat_profile_non_streaming_returns_response` | OpenAI only | `examples/openai/chat.json` (`ThreadedLLMResponse`, streaming overridden to `False`) returns a non-empty completed response |
...
- The Anthropic cache test (Q) requires a large enough context to exceed claude-haiku-4-5's 4096-token cache minimum (this minimum varies by model — newer Haiku/Sonnet/Opus models require more tokens than older ones).
